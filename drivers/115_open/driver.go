...
func (d *Open115) Init(ctx context.Context) error {
	...
	// 初始化 115 缩略图客户端（如果配置了 cookie 或 QRCode token）
	if d.Addition.Cookie115 != "" || d.Addition.QRCodeToken115 != "" {
		if err := InitThumbClient(d.Addition.Cookie115, d.Addition.QRCodeToken115, d.Addition.QRCodeSource115); err != nil {
			utils.Log.Warnf("Failed to init 115 thumb client: %v", err)
		}
	}
...
func (d *Open115) List(ctx context.Context, dir model.Obj, args model.ListArgs) ([]model.Obj, error) {
	...
	// 如果配置了 115 cookie，尝试获取缩略图
	if thumbClient != nil && len(res) > 0 {
		// 收集所有非目录文件的 ID
		var fileIDs []string
		for _, obj := range res {
			if !obj.IsDir() {
				fileIDs = append(fileIDs, obj.GetID())
			}
		}

		// 批量获取缩略图
		thumbs, err := thumbClient.GetThumbsByIDs(fileIDs)
		if err == nil && len(thumbs) > 0 {
			// 用 ObjWithThumb 包装，替换原有对象
			var newRes []model.Obj
			for _, obj := range res {
				if thumbURL, ok := thumbs[obj.GetID()]; ok && thumbURL != "" {
					newRes = append(newRes, &ObjWithThumb{
						Obj:     obj.(*Obj),
						ThumbURL: thumbURL,
					})
				} else {
					newRes = append(newRes, obj)
				}
			}
			res = newRes
		}
	...
	// 默认按文件名升序排序（与 115 逆向驱动一致）
	model.SortFiles(res, "name", "asc")
